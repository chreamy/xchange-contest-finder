    ...
    def classify_contest(row):
        ...
        for category, embeddings in category_embeddings.items():
            if title_embedding is not None and detail_embedding is not None:
                ...
                cos_sim = util.cos_sim(detail_embedding, embeddings)
                k = min(20, cos_sim.size(0))
                top_cos_sim = torch.topk(cos_sim, k).values
                detail_similarity = (top_cos_sim ** 2).mean().item()
...
categories = {
    "程式比賽": ["算法", "編碼", "黑客","黑客松", "軟件", "除錯", "程式語言", "開發", "資料結構", "演算法", "安全性"],
    "文學創作": ["詩歌", "小說", "散文", "出版", "寫作", "自傳"],
    "文學獎": ["文學", "詩歌", "小說", "散文", "批評", "文藝", "理論", "評論","金句"],
    "繪畫": ["寫生","繪畫","藝術","美術","插畫"],
    "平面設計": ["視覺", "版面", "商業設計", "宣傳", "品牌", "圖形設計", "印刷", "排版", "廣告設計", "視覺藝術"],
    "產品設計": ["工業設計", "原型", "使用者體驗", "功能", "包裝", "產品開發", "設計思維", "材料科學", "人機互動", "消費者需求"],
    "多媒體製作": ["剪輯", "動畫", "音效", "視頻", "3D", "多媒體設計", "影片", "音樂", "後製", "數位", "影視","AI"],
    "攝影比賽": ["攝影", "肖像", "風景", "野生", "展覽", "攝影師", "視覺"],
    ...
    "創新創業企劃賽": ["企劃", "市場策略", "初創企業", "投資", "商業模式", "創新管理", "商業發展", "技術創新", "企業精神", "市場進入"],
    "投資競賽": ["股票", "財務規劃", "策略", "預測", "行銷", "投資", "分析", "資本", "經濟", "金融"],
